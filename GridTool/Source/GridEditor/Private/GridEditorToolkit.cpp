...
void FGridEditorToolkit::InitEditor()
{
...
void FGridEditorToolkit::RegisterTabSpawners(const TSharedRef<FTabManager>& InTabManager)
{
	WorkspaceMenuCategory = InTabManager->AddLocalWorkspaceMenuCategory(INVTEXT("Grid Editor Window"));
	...
	// TODO continue initializing the window
}

void FGridEditorToolkit::UnregisterTabSpawners(const TSharedRef<FTabManager>& InTabManager)
{
	InTabManager->UnregisterTabSpawner("ViewportTab");
	InTabManager->UnregisterTabSpawner("DetailsTab");
}

FEditorModeTools& FGridEditorToolkit::GetEditorModeManager() const
{
	check(EditorModeManager.IsValid());
	return *EditorModeManager.Get();
}

FText FGridEditorToolkit::GetToolkitToolTipText() const
{
	return INVTEXT("Helper Window to Visualize a Game Grid");
}

bool FGridEditorToolkit::IsAssetEditor() const
{
	return false;
}

const TArray<UObject*>* FGridEditorToolkit::GetObjectsCurrentlyBeingEdited() const
{
	// TODO-AB revisit that when we have the actual assets
	static TArray<TObjectPtr<UObject>> EditingObjects;
	return &ObjectPtrDecay(EditingObjects);
}

void FGridEditorToolkit::AddMenuExtender(TSharedPtr<FExtender> Extender)
{
	GetGetEditorToolkitHost()->GetMenuExtenders().Add(Extender);
}

TSharedPtr<FExtensibilityManager> FGridEditorToolkit::GetSharedMenuExtensibilityManager()
{
...
TSharedPtr<FExtensibilityManager> FGridEditorToolkit::GetSharedToolbarExtensibilityManager()
{
...
TSharedPtr<SGridEditorToolkitHost> FGridEditorToolkit::GetGetEditorToolkitHost()
{
	return StaticCastSharedPtr<SGridEditorToolkitHost>(ToolkitHost.Pin());
}

void FGridEditorToolkit::OnPersistLayout(const TSharedRef<FTabManager::FLayout>& Layout)
{
